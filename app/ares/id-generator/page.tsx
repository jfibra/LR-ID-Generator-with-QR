...
export default function AresIdGenerator() {
  ...
  const logIdGeneration = useCallback(
    async (action: "access" | "front_download" | "back_download") => {
      ...
      try {
        console.log(`Logging Ares ID generation action: ${action}`)
        ...
        if (result.success) {
          console.log(`Successfully logged Ares ${action}:`, result.data)
        } else {
          console.error(`Failed to log Ares ${action}:`, result.message)
        }
      } catch (error) {
        console.error(`Failed to log Ares ID generation (${action}):`, error)
      }
    },
  ...
  useEffect(() => {
    const storedData = sessionStorage.getItem("aresMemberData")
    ...
    if (storedData) {
      try {
        ...
        console.error("Error parsing Ares member data:", error)
      }
    } else {
      router.push("/ares")
    }
  ...
  const handleDownload = async (isFront: boolean) => {
    ...
    try {
      ...
      link.download = `ares-lr-id-${memberData.memberid}-${isFront ? "front" : "back"}.png`
      link.href = tempCanvas.toDataURL("image/png", 1.0)
      document.body.appendChild(link)
      link.click()
      document.body.removeChild(link)
    } catch (error) {
      console.error("Error generating Ares download:", error)
    ...
    <div className="min-h-screen bg-gray-100">
      <header className="bg-white shadow-sm">
        <div className="container mx-auto px-4 py-4 flex items-center justify-between">
          <div className="flex items-center">
            ...
            <h1 className="text-xl font-bold ml-4 hidden md:block">Ares ID Generator</h1>
          </div>
          <button onClick={() => router.push("/ares")} className="text-gray-600 hover:text-gray-900 flex items-center">
            <svg
              xmlns="http://www.w3.org/2000/svg"
      ...
      {qrCodeContent && (
        <QRCodeRenderer
          ...
          key={`ares-download-${settings.qrCodeSize.width}-${settings.qrCodeSize.height}-${settings.qrCodeModuleShape}-${settings.qrCodeEyeShape}-${settings.qrCodeCornerRadius}-${settings.qrCodeErrorCorrection}-${settings.qrCodeMargin}`}
        />
      )}
    </div>
  )
